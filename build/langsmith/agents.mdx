...
A workspace holds your agents, each agent's traces divide across the same four [environments](/langsmith/agent-environments), and every trace belongs to one environment of one agent. Given a trace, the application that produced it and the deployment it ran on are both already known.
...
A third value also appears: the UUID in the browser address bar. That identifies the record, and tracing configuration does not take it.
