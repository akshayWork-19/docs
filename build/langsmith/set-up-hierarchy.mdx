...
{/*
  DOC-1597. The three parentheticals below are verbatim from the model
  descriptions on workload-isolation.mdx, which owns the taxonomy: 'This model
  (single workspace per team)', 'In this model (multiple teams per workspace)'
  and 'In this model (multiple workspaces per team)'. Change both pages
  together. Nothing enforces this. Grep for 'workspace per team' to find both
  sites; it also matches alt text in byoc-usage.mdx, which describes a diagram
  and is not part of the pair.
*/}
...
What the group is called depends on which information architecture your workspace uses, and the sidebar tells you which one you are on. An agent-based workspace calls it an [agent](/langsmith/agents) and selects it from the agent picker in the top bar. A project-based workspace calls it an application and selects it from the **Application** picker in the sidebar.
