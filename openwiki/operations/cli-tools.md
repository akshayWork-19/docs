...
The repository has two command layers. Make targets are the checkout-facing interface for setup, building, validation, sample workflows, and skill linking. The Python CLI supplies the underlying documentation build, preview, migration, and link-aware move operations. Author in `src/`; `build/` is disposable Mintlify input generated by the pipeline and must not be edited.

## Entry points and dependency boundaries

The project registers `docs = "pipeline.cli:main"`. From a checkout, use `uv run pipeline <command>`; after installation, `docs <command>` invokes the same entry point. `make dev` and `make build` run `npm install`, set the repository on `PYTHONPATH`, and call the module CLI.
...
`make install` runs `uv sync --all-groups`, `npm install`, `npm install -g mint@latest`, and `make skills`. It consequently needs package registries and installs the Mint CLI globally. Run `make help` for the maintained target list.

### Deterministic checkout work

After the needed Python and Node dependencies are already installed, these operations transform or inspect the checkout without requiring Mintlify credentials, a network service, or provider credentials:

- `uv run pipeline build` performs a full local source-to-`build/` transformation.
- `uv run pipeline migrate ... --dry-run` and `uv run pipeline mv ... --dry-run` preview source transformations without writes.
- `make test` runs pytest with network sockets disabled except Unix sockets; use `TEST_FILE` to narrow its `tests/unit_tests` default.
- `make code-snippets`, `make check-cross-refs`, and the formatting, lint, Markdown, and prose checks operate on local files. `make lint_prose` may first download the Vale binary if its pinned local copy is absent.

Do not equate every Make wrapper with an offline command: `make build` and `make dev` each run `npm install`, which can contact the npm registry. For a repeatable already-provisioned build, invoke the underlying pipeline command directly.

### Commands that require Mint, network access, credentials, or services

| Boundary | Commands | Operational expectation |
| --- | --- | --- |
| Mint CLI and local preview | `make dev`, `make broken-links*`, `make check-openapi`, `make export` | Require the separately installed global `mint`; preview binds a local port. Mint commands run from generated `build/`. |
| Mint export entitlement | `make export`, `make htmltest`, `make export-htmltest` | Export requires a Mint CLI with `mint export`, Node below 25, and an Enterprise Mintlify plan. `htmltest` also needs an export archive, `htmltest`, and `unzip`. |
| Live code samples | `make test-code-samples` | Runs real programs and may require language toolchains, provider credentials, network access, or local services. |
...
This shows the clean initial build and the focused update path used by local preview.
...
| `make broken-links` | Builds, then runs `mint broken-links` from `build/`; filters known deployment/OpenAPI and standalone-snippet noise. | Check generated routes, redirects, and links. |
...
| `make test` | Runs pytest with socket isolation. | Pipeline and script behavior; narrow with `TEST_FILE`. |
| `make lint`, `make format`, `make format-check` | Check or change Python formatting, lint, type, and spelling state. | Tooling or Python changes. |
| `make lint_md`, `make lint_md_fix`, `make lint_prose` | Markdownlint or apply Markdown fixes; Vale checks `FILES` or `src/`. | Markdown and prose changes. |
...
The built-site link targets run Mintlify from `build/` after a build. They filter deployment-generated OpenAPI and standalone-snippet noise, and fail only if filtered output still contains reported link entries. Both validate redirects; the anchor variant also checks anchors. `make export` requires a Mint CLI that supports `mint export`, Node below 25, and an Enterprise Mintlify plan. `make htmltest` uses a supplied archive and checks external URLs only, so it does not replace generated-site navigation validation.

## Code samples, snippets, and traces

`make code-snippets` is a local generation pipeline with two owned output trees:
...
See [Mintlify Integration](/openwiki/integrations/mintlify.md), [Local Development Workflow](/openwiki/workflows/local-development.md), [Testing Overview](/openwiki/testing/test-overview.md), and [Code Sample Lifecycle](/openwiki/workflows/code-sample-lifecycle.md) for renderer boundaries, preview mechanics, the broader validation matrix, and sample authoring.
